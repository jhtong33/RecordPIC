...
class IOpenmallSpider(scrapy.Spider):
    ...
    def __init__(self, url=None, runner=None, **kwargs):
        self.target_url = url
        self.runner = runner
        super(IOpenMall, self).__init__(**kwargs)
    ...
    def parse_tier3(self, response):
        try:
            ...
            first_menu_text = response.xpath('//div[contains(@class, "top_menu_list")]/a/text()').get()
            sec_menu_text = response.xpath('//ul[@class="top_menu_sec"]/li[@class="top_menu_sec_sort"]//div/a/text()').get()
            print(first_menu_text, sec_menu_text)
            ...
            if len(for_menus) > 0:
                for for_menu in for_menus:
                    ...
                    # category_text = category.css('').get()
                    # category_link = category.css('div a::attr(href)').get()
                    print(for_text, for_link)
                    tier_dict = {}
                    fif_menus = for_menu.css('ul > li:nth-child(n)')
                    if len(fif_menus) > 0: 
