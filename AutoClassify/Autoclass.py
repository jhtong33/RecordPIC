...
def find_best_cluster(method, df_4cluster, pma, savedir, catobj_Pos, selectMethod, Gmin=2, Gmax=10):
    ...
    if method.casefold() == "KMeans".casefold():
        ...
        for n_clusters in range_cluster:
            km = KMedoids(n_clusters=n_clusters,  random_state=50)
            labels = km.fit_predict(df_4cluster)
            scores["elbow"].append(km.inertia_)
            scores["calinski-harabasz"].append(calinski_harabasz_score(df_4cluster, labels))
            scores["davies-bouldin"].append(davies_bouldin_score(df_4cluster, labels))
        ...
        for n_clusters in range_cluster:
            kproto = KPrototypes(n_clusters=n_clusters, init="Huang", max_iter=10,  random_state=50)
        ...
        for n_clusters in range_cluster:
            kmodes = KModes(n_clusters=n_clusters, init = "Huang", random_state=50)
    ...
    for m, automethod in enumerate(bestscores.keys()):
        ...
        axs[m].scatter(bestscores[automethod], scores[automethod][bestscores[automethod][0]-2], color="r", marker="*")
